...
public class JsonClient6802 {
    ...
    public JsonClient6802(String ip, int port, SocketDataCallback socketDataCallback, SocketStateCallback socketStateCallback) {
        this.ip = ip;
        this.port = port;
...
//                Log.d(TAG, "Heartbeat Running 心跳运行");
            }
//            Log.d("HeartbateThread  心跳线程 ", "exit 退出");
        }
    }
    ...
    private boolean connect() {
        try {
            ...
            if (DEBUG) {
                Log.e(TAG, "套接字连接成功 == Socket Connect success!");
            }
            ...
            Log.e(TAG, "套接字连接错误 == Socket Connect Error!");
            return false;
        }
        return true;
    }
    ...
    private class ReceiveThread extends Thread {
        ...
        public void run() {
            ...
            Log.d(TAG, "读取线程退出read thread quit");
        }
    }
