...
import org.easydarwin.client.JsonClient6802;
...
//                sendCmd(CV.TX2SPK_VOLUME, String.valueOf(new SharedPreferencesUtil().getTx2spkVolume()));
            ...
            case 0x02:
                ...
                if (DEBUG) {
                    Log.e(TAG, "run:银临 通信连接断开   " + HideyBarUtils.getTime());
                }
                break;
            default:
                break;
        ...
        try {
            ...
            SingletonInternalClass.getInstance().getSingleThreadExecutor().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    mJsonClient6802.sendBytes(ByteConvertUtil.addBytes(ByteConvertUtil.intToBytes(bytes.length), bytes));
                }
            });
